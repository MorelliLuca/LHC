...
class Particle {
  ...
  static constexpr int maxNumParticleType{
      10};  // Max number of Types of particles
  // Non static
  int index_;
  double Px_, Py_, Pz_;
 ...
 public:
  ...
  // Non static
  int getIndex() const;                // Returns the index of the type
  void setParticle(int index);         // Sets the type by index
  void setParticle(std::string name);  // Sets the type by name
